...
class FlashingCoin(pygame.sprite.Sprite):
    ...
    def load_frames(self, frame_rects):
        ...
        for frame_rect in frame_rects:
            self.frames.append(tools.get_image(sheet, *frame_rect, (0, 0, 0), constants.BG_MULTI))

    def update(self):
        self.current_time = pygame.time.get_ticks()
        frame_durations = [375, 125, 125, 125]
        # frame_durations = [60000, 20000, 20000, 20000]
        # frame_durations = [3, 1, 1, 1]
        gap = self.current_time - self.timer
        if self.timer == 0:
            self.timer = self.current_time
        # elif (self.current_time - self.timer) > frame_durations[self.frame_index]:
        elif (gap) > frame_durations[self.frame_index]:
            self.frame_index += 1
            self.frame_index %= 4
            self.timer = self.current_time
